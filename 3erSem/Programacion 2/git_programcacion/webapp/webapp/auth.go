...
func (s *authService) getLogin(c *gecko.Context) error {
	...
	return c.Render(200, "layouts/login", nil)
}
...
func (s *authService) PersistirSesiones() {
	...
	data, err := json.MarshalIndent(s.sesiones, "", "\t")
	if err != nil {
		op.Err(err).Log()
	}
	err = os.WriteFile("sesiones.json", data, 0600)
	if err != nil {
		op.Err(err).Log()
	}
}
